...
import { getCollectionBySlug, getBadgeConfig } from '../../data/formation';

const CollectionPage = () => {
    // Récupérer le slug de la collection depuis l'URL
    const { collectionSlug } = useParams();
    
    // États pour le panier et les notifications
    const [cart, setCart] = useState([]);
    const [notification, setNotification] = useState(null);
    
    // Trouver la collection correspondante en utilisant la fonction utilitaire
                ...
                <div className="container mx-auto px-4 sm:px-6 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-sm text-gray-500">
                            {collection.items.length} ressource{collection.items.length > 1 ? 's' : ''}
                        </div>
                    </div>
                </div>
            </div>
